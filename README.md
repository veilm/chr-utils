...
- Global Note Bindings: configure custom Alt+Q letters that save a chosen exact-URL note, append the page URL to one or more absolute file paths through the authenticated local server, or do both; custom letters override built-in quick actions; the settings panel can also export every page note to JSON or import a backup while either keeping current notes (the default) or overwriting matching rules
